...
export default function QuizPage({ onNavigate }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    if (res) {
      // next_step is the reliable pass indicator — after passing, generate_quiz
      // runs for the next week and resets quiz_passed to null in the state snapshot.
      ...
      // Unlock chatbot once quiz is concluded (pass or roadmap-adjust terminal state)
  ...
  const handleRetry = () => {
    ...
    setResult(null);
    clearError();
    // Stay in quiz mode — user is still attempting the quiz
  };
  ...
  // ── Pass result ────────────────────────────────────────────────────────────
  if (result?.passed) {
    const percentage = Math.round((result.score / 100) * 100);
    const advancedWeek = result.nextWeek > result.weekCompleted;
    return (
      <div className="space-y-6">
        <PageHeader week={result.weekCompleted} focus={currentWeekData?.focus ?? null} />
        <div className="bg-white rounded-xl border border-green-100 p-10 flex flex-col items-center text-center gap-6">
          <div className="w-20 h-20 rounded-full bg-green-50 flex items-center justify-center">
            <Trophy className="w-10 h-10 text-green-500" />
          </div>
          <div className="space-y-1">
            <h3 className="text-2xl font-bold text-slate-800">¡Aprobaste!</h3>
            <p className="text-slate-500 text-sm">Quiz de la Semana {result.weekCompleted} completado</p>
          </div>
          <div className="flex items-center gap-8 py-4 px-8 bg-green-50 rounded-xl">
            <ScoreStat label="Puntuación" value={`${percentage}%`} color="text-green-600" />
            <div className="w-px h-10 bg-green-200" />
            <ScoreStat label="Correctas" value={`${Math.round(result.score / 20)}/5`} color="text-green-600" />
          </div>
          {advancedWeek && (
            <div className="flex items-center gap-2 text-sm font-medium text-indigo-600 bg-indigo-50 px-4 py-2.5 rounded-lg">
              <ArrowRight className="w-4 h-4" />
              Avanzas a la Semana {result.nextWeek}
            </div>
          )}
          {session.strongSkills.length > 0 && (
            <SkillPills
  ...
  if (result && !result.passed) {
    ...
    const weekModules = currentWeekData?.modules ?? [];
    return (
      <div className="space-y-6">
        <PageHeader week={result.weekCompleted} focus={currentWeekData?.focus ?? null} />
        <div className="bg-white rounded-xl border border-red-100 p-8 flex flex-col items-center text-center gap-6">
          <div className="w-16 h-16 rounded-full bg-red-50 flex items-center justify-center">
            <XCircle className="w-8 h-8 text-red-500" />
          </div>
          <div className="space-y-1">
            <h3 className="text-xl font-bold text-slate-800">Quiz no aprobado</h3>
            <p className="text-sm text-slate-500">Necesitas al menos 70% para pasar</p>
          </div>
          <div className="flex items-center gap-8 py-4 px-8 bg-red-50 rounded-xl">
            <ScoreStat label="Obtenido" value={`${percentage}%`} color="text-red-500" />
            <div className="w-px h-10 bg-red-200" />
            <ScoreStat label="Correctas" value={`${Math.round(result.score / 20)}/5`} color="text-red-500" />
          </div>

          {(weekModules.length > 0 || session.weakSkills.length > 0) && (
            <div className="w-full text-left space-y-4 border-t border-slate-100 pt-4">
              {weekModules.length > 0 && (
                <div>
                  <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">
                    Repasa los temas de esta semana
                  </p>
                  <div className="space-y-2">
                    {weekModules.map((mod) => (
                      <div
                        key={mod.module_number}
                        className="flex items-start gap-2 text-sm text-slate-600 bg-amber-50 px-3 py-2.5 rounded-lg border border-amber-100"
                      >
                        <BookOpen className="w-3.5 h-3.5 text-amber-500 flex-shrink-0 mt-0.5" />
                        <div>
                          <p className="font-medium text-slate-700">{mod.name}</p>
                          <p className="text-xs text-slate-500 mt-0.5">{mod.objective}</p>
                        </div>
                      </div>
                    ))}
                  </div>
...
function SkillPills({ label, skills, chipClass }: { label: string; skills: string[]; chipClass: string }) {
  return (
    <div className="w-full text-left">
      <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">{label}</p>
      <div className="flex flex-wrap gap-2">
        {skills.map((skill) => (
          <span key={skill} className={`text-xs px-2.5 py-1 rounded-full border ${chipClass}`}>
            {skill}
          </span>
        ))}
      </div>
